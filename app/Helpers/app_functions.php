...
{
    return PostCategory::where("show_on_dashboard" , 1)->orderby("name" , "asc")->get(["id" , "name"]);
}

function developer()
{
    return User::where("email" , Constants::DEV_EMAIL)->first();
}

function isDev()
{
    return optional(auth()->user())->email == Constants::DEV_EMAIL;
}
